...
import { clickTab, fillVerificationTab } from './part-b-portal';
...
export async function fillVerificationAndSubmitEvc(
  page: Page,
  ...
  await api.reportProgress(applicationId, 'verification_fill');
  await clickTab(page, selectors.partB.tabs.verification);

  const signatoryName = `${formData.promoter.firstName} ${formData.promoter.lastName}`.trim();
  await fillVerificationTab(page, signatoryName, formData.verification.place);
  ...
  const evcBtn = page.locator(selectors.partB.submitEvc).first();
  await evcBtn.waitFor({ state: 'visible', timeout: 15000 }).catch(() => {});
  await evcBtn.click().catch(async () => {
...
/** @deprecated use fillVerificationAndSubmitEvc */
...
export async function runSubmitFlow(
  ...
  const submitResult = await fillVerificationAndSubmitEvc(
    page,
    formData,
    applicationId,
    jobId,
